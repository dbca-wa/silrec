<template>
    <div class="treatment-form">
        <div v-if="$route.query.debug?.toLowerCase() === 'true'">
            src/components/internal/treatments/treatments_form.vue
        </div>
                    ...
                    <div v-if="!extrasCollapsed" class="card-body">
                        ...
                        >
                            <span class="text-muted"
                                >Additional treatment information and
                                attributes</span
                            >
                            <router-link
                                ...
                                class="btn btn-outline-primary btn-sm"
                            >
                                <i class="bi bi-plus"></i> Add Treatment Extra
                                Details
                            </router-link>
                        </div>
...
export default {
    ...
    methods: {
        ...
        validateForm() {
            ...
            if (
                this.treatmentData.plan_mth &&
                (this.treatmentData.plan_mth < 1 ||
                    this.treatmentData.plan_mth > 12)
            ) {
                swal.fire({
                    icon: 'error',
                    title: 'Validation Error',
            ...
            if (
                this.treatmentData.pct_area &&
                (this.treatmentData.pct_area < 0 ||
                    this.treatmentData.pct_area > 100)
            ) {
                swal.fire({
                    icon: 'error',
                    title: 'Validation Error',
        ...
        async handleSaveError(error) {
            ...
            if (error.message && error.message.includes('403')) {
                errorMessage =
                    'You do not have permission to create or update treatments. Please contact an administrator.';
