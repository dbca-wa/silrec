        ...
        <div class="col-md-6">
          <div class="form-group mb-3">
            ...
            <div class="searchable-select">
              ...
              />
              <div v-if="showObjectiveDropdown" class="dropdown-options">
                <div 
                ...
                >
                  <strong>{{ objective.obj_code }}</strong> - {{ objective.description || 'No description' }}
                </div>
                <div v-if="filteredObjectives.length === 0" class="dropdown-option no-results">
                  No objectives found
                </div>
              </div>
            </div>
          ...
          <div class="form-group mb-3">
            ...
            <div class="searchable-select">
              <input
                ...
                @input="filterSpecies"
              />
              <div v-if="showSpeciesDropdown" class="dropdown-options">
                <div 
                ...
                >
                  <strong>{{ species.species }}</strong> - {{ species.short_description || species.full_description || 'No description' }}
                </div>
                <div v-if="filteredSpecies.length === 0" class="dropdown-option no-results">
                  No species found
                </div>
              </div>
            </div>
        ...
        <div class="col-md-6">
          <div class="form-group mb-3">
            ...
            <div class="searchable-select">
              ...
              />
              <div v-if="showRegenMethodDropdown" class="dropdown-options">
                <div 
                ...
                >
                  <strong>{{ method.regen_method }}</strong> - {{ method.description || 'No description' }}
                </div>
                <div v-if="filteredRegenMethods.length === 0" class="dropdown-option no-results">
  ...
  methods: {
    ...
    initializeSearchFields() {
      ...
      if (this.formData.obj_code && this.lookups.objectives.length > 0) {
        const currentObj = this.lookups.objectives.find(obj => obj.obj_code === this.formData.obj_code);
        this.objectiveSearch = currentObj ? `${currentObj.obj_code} - ${currentObj.description || ''}` : this.formData.obj_code;
      }
      
      if (this.formData.species && this.lookups.species.length > 0) {
        const currentSpecies = this.lookups.species.find(sp => sp.species === this.formData.species);
        this.speciesSearch = currentSpecies ? `${currentSpecies.species} - ${currentSpecies.short_description || currentSpecies.full_description || ''}` : this.formData.species;
      }
      
      if (this.formData.regen_method && this.lookups.regeneration_methods.length > 0) {
        const currentMethod = this.lookups.regeneration_methods.find(method => method.regen_method === this.formData.regen_method);
        this.regenMethodSearch = currentMethod ? `${currentMethod.regen_method} - ${currentMethod.description || ''}` : this.formData.regen_method;
      }
    },
    ...
    filterObjectives() {
      ...
      this.filteredObjectives = this.lookups.objectives.filter(obj => 
    ...
    filterSpecies() {
      ...
      this.filteredSpecies = this.lookups.species.filter(sp => 
        sp.species.toLowerCase().includes(searchTerm) ||
        (sp.short_description && sp.short_description.toLowerCase().includes(searchTerm)) ||
        (sp.full_description && sp.full_description.toLowerCase().includes(searchTerm))
      );
    },
    ...
    filterRegenMethods() {
      ...
      this.filteredRegenMethods = this.lookups.regeneration_methods.filter(method => 
        method.regen_method.toLowerCase().includes(searchTerm) ||
    ...
    getLookupDisplayValue(fieldName, value) {
      const lookupMap = {
        'obj_code': { data: this.lookups.objectives, key: 'obj_code', display: 'description' },
        'species': { data: this.lookups.species, key: 'species', display: 'short_description' },
        'regen_method': { data: this.lookups.regeneration_methods, key: 'regen_method', display: 'description' }
      };
      ...
      return item ? `${item[config.key]} - ${item[config.display] || 'No description'}` : value;
    }
  },
  mounted() {
    this.loadLookups();
