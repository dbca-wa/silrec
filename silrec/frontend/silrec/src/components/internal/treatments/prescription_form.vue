<template>
    <div class="prescription-form">
        <div v-if="$route.query.debug?.toLowerCase() === 'true'">
            src/components/internal/treatments/prescription_form.vue
        </div>
        ...
        <div class="card">
            ...
            -->
                    ...
                    <div class="row mt-3">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="canReschedule"
                                    >Can Reschedule</label
                                >
                                <select
                                    id="canReschedule"
    ...
    methods: {
        ...
        async savePrescription() {
            ...
            try {
                ...
                if (this.isNew && this.treatmentId) {
                    await this.updateTreatmentWithPrescription(
                        responseData.prescription_id
                    );
                }
        ...
        async updateTreatmentWithPrescription(prescriptionId) {
            try {
                ...
                if (!response.ok) {
                    throw new Error(
                        `Failed to update treatment with prescription: ${response.status}`
                    );
                }

                console.log(
                    'Treatment updated with prescription ID:',
                    prescriptionId
                );
            } catch (error) {
                console.error(
                    'Error updating treatment with prescription:',
                    error
                );
        ...
        validateForm() {
            ...
            if (
                !this.prescriptionData.sequence ||
                this.prescriptionData.sequence < 1
            ) {
                swal.fire({
                    icon: 'error',
                    title: 'Validation Error',
            ...
            if (
                this.prescriptionData.area_pct &&
                (this.prescriptionData.area_pct < 0 ||
                    this.prescriptionData.area_pct > 100)
            ) {
                swal.fire({
                    icon: 'error',
                    title: 'Validation Error',
        ...
        async handleSaveError(error) {
            ...
            if (error.message && error.message.includes('403')) {
                errorMessage =
                    'You do not have permission to create or update prescriptions. Please contact an administrator.';
