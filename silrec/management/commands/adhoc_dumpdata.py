...
class Command(BaseCommand):
    ...
    def handle(self, *app_labels, **options):
        ...
        def get_objects(count_only=False):
            ...
            from silrec.components.forest_blocks.models import Polygon, Cohort, Treatment, TreatmentXtra, Prescription

            proposal_id = 5

            #qs1 = Proposal.objects.filter(pk=proposal_id)
            qs2 = Polygon.objects.filter(proposal_id=5)
            qs3 = Cohort.objects.filter(assignchttoply__polygon__in=qs2)
            qs4 = Treatment.objects.filter(cohort__in=qs3)
            qs5 = TreatmentXtra.objects.filter(treatment__in=qs4)
            qs6 = Prescription.objects.filter(treatment__in=qs4)

            yield from chain(qs2, qs3, qs4, qs5, qs6)

        def _get_objects(count_only=False):
            """
