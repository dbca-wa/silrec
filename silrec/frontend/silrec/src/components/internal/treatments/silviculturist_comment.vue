<template>
  <div class="silviculturist-comment">
    <div v-if="$route.query.debug?.toLowerCase() === 'true'">src/components/internal/treatments/silviculturist_comment.vue</div>
      ...
      <table class="table table-striped table-hover">
        ...
        <tbody>
          <tr v-for="comment in comments" :key="comment.s_comment_id">
            <td class="comment-text">{{ truncateText(comment.comment, 50) }}</td>
            <td>{{ comment.required_action || 'N/A' }}</td>
            <td>
              <span v-if="comment.action_complete" class="badge bg-success">
    ...
    <div class="modal fade" :class="{ 'show d-block': showCommentModal }" tabindex="-1" v-if="showCommentModal">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ editingComment ? 'Edit Silviculturist Comment' : 'Add New Silviculturist Comment' }}</h5>
          ...
          <div class="modal-body">
            <form @submit.prevent="saveComment">
              ...
              <div class="row mt-3">
                <div class="col-md-6">
                  <div class="form-group">
                    <label for="requiredAction" class="form-label">Required Action</label>
                    <input
                      id="requiredAction"
                      v-model="commentData.required_action"
                      type="text"
                      class="form-control"
                      :readonly="readOnly"
                ...
                <div class="col-md-6">
                  <div class="form-group">
                    <label for="actionComplete" class="form-label">Action Complete Date</label>
                    <input
                      id="actionComplete"
                      v-model="commentData.action_complete"
                      type="date"
                      class="form-control"
                      :readonly="readOnly"
                    />
              ...
              <div class="row mt-3">
                <div class="col-md-6">
                  <div class="form-group">
                    <label for="easting" class="form-label">Easting</label>
                    <input
                      id="easting"
                      v-model.number="commentData.easting_note_taken"
                      type="number"
                      class="form-control"
                      :readonly="readOnly"
                ...
                <div class="col-md-6">
                  <div class="form-group">
                    <label for="northing" class="form-label">Northing</label>
                    <input
                      id="northing"
                      v-model.number="commentData.northing_note_taken"
                      type="number"
                      class="form-control"
                      :readonly="readOnly"
          ...
          <div class="modal-footer">
            ...
            >
              ...
              {{ saving ? 'Saving...' : (editingComment ? 'Update Comment' : 'Save Comment') }}
            </button>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    async deleteComment(comment) {
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`${api_endpoints.silviculturist_comments}${comment.s_comment_id}/`, {
            method: 'DELETE',
            headers: {
              'X-CSRFToken': this.getCSRFToken()
            }
    ...
    editComment(comment) {
      ...
      this.commentData = {
        ...
        action_complete: comment.action_complete ? comment.action_complete.split('T')[0] : null,
    ...
    async handleSaveError(error) {
      ...
      if (error.message && error.message.includes('403')) {
        errorMessage = 'You do not have permission to create or update comments. Please contact an administrator.';
