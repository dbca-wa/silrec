<template>
    <div class="report-generator">
        <div v-if="$route.query.debug?.toLowerCase() === 'true'">
            src/components/internal/reports/report_generator.vue
        </div>
        ...
        <div class="card">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="reportSelect" class="form-label"
                                    >Report Type *</label
                                >
                                <select
                                    id="reportSelect"
                        ...
                        >
                            <div class="form-group">
                                <label :for="param.name" class="form-label">
                                    ...
                                    <span
                                        v-if="
                                            param.field_type === 'year_enhanced'
                                        "
                                        class="badge bg-secondary ms-1"
                                    >
                                        <i class="bi bi-calendar-range"></i>
                                        Supports: 2024, 2024,2023,2022,
                                        2020-2024
                                    </span>
                                </label>
                                ...
                                >
                                    <option
                                        ...
                                        :value="option"
                                        :selected="
                                            isOptionSelected(param.name, option)
                                        "
                                    >
                                        {{ option }}
                                    </option>
                                ...
                                <div
                                    v-else-if="param.field_type === 'range'"
                                    class="row g-2"
                                >
                                    <div class="col">
                                ...
                                <div
                                    v-if="param.field_type === 'year_enhanced'"
                                    class="form-text text-muted"
                                >
                                    Enter a single year, comma-separated years,
                                    or a year range (e.g., 2020-2024). You can
                                    also combine formats like: 2024,2022-2023
                                </div>
                            </div>
                        </div>
                    </div>
          ...
          -->
                    ...
                    <div
                        v-for="(clause, index) in customClauses"
                        :key="index"
                        class="row mb-3 border p-3 rounded"
                    >
                        <div class="col-md-4">
    ...
    methods: {
        validateEnhancedYear(value) {
            ...
            for (const part of parts) {
                ...
                if (part.includes('-')) {
                    ...
                    if (!this.isValidYear(start) || !this.isValidYear(end))
                        return false;
        ...
        isOptionSelected(paramName, option) {
            return (
                ...
                this.parameters[paramName].includes(option)
            );
        },
        ...
        async loadAvailableReports() {
            ...
            try {
                const response = await fetch(
                    `${api_endpoints.reports}list_reports/`
                );
                if (response.ok) {
        ...
        async fetchAvailableFields(reportId) {
            ...
            try {
                const response = await fetch(
                    `${api_endpoints.reports}${reportId}/available_fields/`
                );
                if (response.ok) {
                    ...
                    console.warn(
                        'Could not load available fields, using empty list'
                    );
                }
            } catch (error) {
        ...
        initializeSelect2() {
            ...
            $('.select2-multiple-tags').each((index, element) => {
                ...
                $element
                    ...
                    .on('change', (e) => {
                        const paramName = e.target.id.replace(
                            'multiselect-',
                            ''
                        );
        ...
        onFieldChange(index) {
            ...
            if (clause.field) {
                if (
                    clause.field.includes('date') ||
                    clause.field.includes('year')
                ) {
                    clause.operator = '=';
                } else if (
                    clause.field.includes('area') ||
                    clause.field.includes('rate')
                ) {
                    clause.operator = '>=';
                }
            }
        ...
        copySqlToClipboard() {
            if (this.previewData && this.previewData.sql) {
                navigator.clipboard
                    .writeText(this.previewData.sql)
                    .then(() => {
            ...
            try {
                ...
                Object.keys(this.parameters).forEach((key) => {
                    ...
                    if (Array.isArray(value)) {
                        ...
                        const paramConfig = this.selectedReport.parameters.find(
                            (p) => p.name === key
                        );
                        if (paramConfig && paramConfig.field_type === 'range') {
                            ...
                            requestData.parameters[key] = value.filter(
                                (v) => v !== '' && v != null
                            );
                        }
                    } else {
                ...
                const validCustomClauses = this.customClauses.filter(
                    (clause) => {
        ...
        async previewReport() {
            ...
            try {
                ...
                if (validCustomClauses.length > 0) {
                    params.append(
                        'custom_clauses',
                        JSON.stringify(validCustomClauses)
                    );
                }
                ...
                if (response.ok) {
                    ...
                    throw new Error(
                        errorData.error || 'Failed to preview report'
                    );
                }
            } catch (error) {
...
.select2-container--bootstrap-5
    .select2-selection--multiple:not(.select2-selection--multiple-selected)
...
.select2-container--bootstrap-5
    .select2-selection--multiple.select2-selection--multiple-selected
    .select2-selection__rendered::before {
    display: none;
}
