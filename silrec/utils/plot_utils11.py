...
class IndividualChartPopup:
    ...
    def open_popup(self):
        ...
        if self.gdf is not None and not self.gdf.empty and hasattr(self.gdf, 'geometry'):
            try:
                self.gdf.plot(ax=ax, color='steelblue', alpha=0.8, edgecolor='black', linewidth=0.5)
    ...
    def maximize_row(self):
        ...
        for i, (original_fig, original_canvas, original_frame, gdf, title, description) in enumerate(self.row_charts_data):
            ...
            if gdf is not None and not gdf.empty and hasattr(gdf, 'geometry'):
                try:
                    gdf.plot(ax=new_ax, color='steelblue', alpha=0.7, edgecolor='black')
                    ...
                    new_ax.text(0.5, 0.5, f'Plot error: {str(e)}', ha='center', va='center', transform=new_ax.transAxes)
                    new_ax.set_title(title, fontsize=12, fontweight='bold')
            else:
        ...
        for col_idx, gdf in enumerate(row_gdfs):
            ...
            if gdf is not None and not gdf.empty and hasattr(gdf, 'geometry'):
                try:
                    ...
                    ax.set_title(chart_title, fontsize=9, fontweight='bold', pad=8)
