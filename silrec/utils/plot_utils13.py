...
def plot_geodataframe(ax, gdf, title):
    """Helper function to plot GeoDataFrame with annotations"""
    if gdf is not None and not gdf.empty and hasattr(gdf, 'geometry'):
        try:
            # Plot the geometries
            gdf.plot(ax=ax, color='steelblue', alpha=0.7, edgecolor='black', linewidth=0.8)

            # Add annotations for polygons
...
class IndividualChartPopup:
    ...
    def open_popup(self):
        ...
        # Plot the GeoDataFrame with annotations
...
class TabMaximizer:
    def __init__(self, root, tab_charts_data, tab_name, tab_description):
        self.root = root
        self.tab_charts_data = tab_charts_data
        self.tab_name = tab_name
        self.tab_description = tab_description
        self.maximized_window = None
        self.maximized_canvases = []

    def maximize_tab(self):
        # Create new window for maximized tab
        self.maximized_window = tk.Toplevel(self.root)
        self.maximized_window.title(f"Maximized View - {self.tab_name}")
        ...
        def configure_scroll_region(event):
            ...
            # Set fixed width for the canvas window
        ...
        # Create title with tab description
        title_frame = tk.Frame(scrollable_frame, bg='white')
        title_frame.pack(fill=tk.X, pady=(0, 20))

        title_label = tk.Label(title_frame, text=f"{self.tab_name} - Maximized View",
                              font=('Arial', 14, 'bold'), bg='white')
        title_label.pack()

        # Add tab description
        if self.tab_description:
            desc_label = tk.Label(title_frame, text=self.tab_description,
        ...
        for i, (original_fig, original_canvas, original_frame, gdf, title, description) in enumerate(self.tab_charts_data):
            ...
            # Create frame for this chart with individual popup button
            ...
            popup_button = tk.Button(chart_container, text="📊",
                                   font=('Arial', 12),
                                   command=lambda g=gdf, t=title, d=description: self.open_individual_chart(g, t, d),
                                   bg='lightgreen', width=3)
            popup_button.pack(anchor='ne', pady=(0, 5))

            # Create chart frame
            chart_frame = tk.Frame(chart_container, relief=tk.RAISED, bd=2, bg='white')
            ...
            # Plot the GeoDataFrame with annotations
            plot_geodataframe(new_ax, gdf, title)
    ...
    def open_individual_chart(self, gdf, title, description):
        chart_popup = IndividualChartPopup(self.maximized_window, gdf, title, description)
...
def create_tabbed_charts(*tab_lists, tab_names=None, tab_descriptions=None, chart_titles=None, chart_descriptions=None):
    ...
    *tab_lists: Variable number of lists, each containing GeoDataFrames for a tab
    ...
    tab_data = {}  # Store chart data by tab
    ...
    for tab_idx, tab_gdfs in enumerate(tab_lists):
        ...
        # Create canvas with vertical scrollbar
        canvas = tk.Canvas(main_container, yscrollcommand=v_scrollbar.set)
        canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        ...
        scrollable_frame.bind("<Configure>", lambda e: configure_scroll_region(e))

        def on_canvas_configure(event, c=canvas, cw=canvas_window):
            c.itemconfig(cw, width=event.width)

        canvas.bind("<Configure>", lambda e: on_canvas_configure(e))
        ...
        # Add maximize tab button
        maximize_btn = tk.Button(desc_frame, text="⤢ Maximize Tab View",
                               font=('Arial', 10, 'bold'),
                               command=lambda t=tab_idx: maximize_tab_popup(t),
                               bg='lightblue')
        maximize_btn.pack(pady=5)

        # Create charts container
        charts_container = tk.Frame(scrollable_frame)
        ...
        for chart_idx, gdf in enumerate(tab_gdfs):
            ...
            chart_container = tk.Frame(charts_container)
            chart_container.grid(row=current_row, column=current_col, padx=10, pady=10, sticky="nsew")
            ...
            # Store chart data for tab maximization
            tab_data[tab_idx].append((fig, chart_canvas, chart_frame, gdf, chart_title, chart_description))

        # Mouse wheel scrolling for the tab
        def _on_mousewheel(event, c=canvas):
            c.yview_scroll(int(-1*(event.delta/120)), "units")

        canvas.bind_all("<MouseWheel>", lambda e: _on_mousewheel(e, canvas))

    def maximize_tab_popup(tab_index):
        if tab_index in tab_data:
            name = tab_names[tab_index] if tab_index < len(tab_names) else f"Tab {tab_index + 1}"
            description = tab_descriptions.get(tab_index, f"Tab {tab_index + 1} spatial analysis")
            maximizer = TabMaximizer(root, tab_data[tab_index], name, description)
            maximizer.maximize_tab()
    ...
    instructions = tk.Label(root,
                          text="Navigation: Click tabs to switch views | Click 📊 for individual chart popup | Click ⤢ to maximize tab | Scroll: Mouse Wheel",
...
def example_usage():
    ...
    try:
        ...
        # Create sample GeoDataFrames for different tabs
        # Tab 1: Urban Areas
        ...
        tab_names = ["Urban Areas", "Rural Districts", "Mixed Zones"]

        tab_descriptions = [
            "Analysis of urban development patterns and city infrastructure",
            "Rural and agricultural land use mapping and analysis",
            "Mixed-use zones showing residential and commercial areas"
        ]
