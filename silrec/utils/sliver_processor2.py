...
class SliverProcessor:
    ...
    def merge_polygons_by_id(self, polygon_ids, area_tolerance=0.0001):
        """
        Merge specified polygons by their IDs.
        """
        ...
        if not merged_geometry.is_valid:
            raise ValueError("Merged geometry is invalid")
        ...
        if area_diff / original_merged_area > area_tolerance:
            raise ValueError(f"Area preservation check failed for merge: {area_diff/original_merged_area*100:.4f}% difference")
        ...
        print(f"  Total area difference: {total_area_diff:.10f} ({total_area_diff/original_area*100:.6f}%)")
