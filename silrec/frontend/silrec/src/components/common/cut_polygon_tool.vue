      ...
      <div class="cut-panel-content">
        ...
        <div v-if="cutStep < 2" class="two-column-layout">
          ...
          <div class="steps-column">
            <div class="step-row" :class="{ active: currentStep === 1, completed: selectedPolygon }">
            ...
            <div class="step-row" :class="{ active: currentStep === 2, completed: point1 }">
              <div class="step-content">
            ...
            <div class="step-row" :class="{ active: currentStep === 3, completed: point2 }">
              <div class="step-content">
          ...
          <div class="values-column">
            <div class="value-row" :class="{ 'has-value': selectedPolygonId }">
              <span class="value-label">ID:</span>
              <span class="value-content">{{ selectedPolygonId || '—' }}</span>
            </div>
            <div class="value-row" :class="{ 'has-value': point1 }">
              <span class="value-label">P1:</span>
              <span class="value-content">{{ point1 ? `[${point1[0].toFixed(4)}, ${point1[1].toFixed(4)}]` : '—' }}</span>
            </div>
            <div class="value-row" :class="{ 'has-value': point2 }">
              <span class="value-label">P2:</span>
              <span class="value-content">{{ point2 ? `[${point2[0].toFixed(4)}, ${point2[1].toFixed(4)}]` : '—' }}</span>
            </div>
          </div>
        </div>
        ...
        <div v-if="cutStep === 2" class="two-column-layout post-cut">
          ...
          <div class="values-column">
            ...
            <div class="value-row has-value">
              <span class="value-label">P1:</span>
              <span class="value-content">[{{ point1[0].toFixed(4) }}, {{ point1[1].toFixed(4) }}]</span>
            </div>
            <div class="value-row has-value">
              <span class="value-label">P2:</span>
              <span class="value-content">[{{ point2[0].toFixed(4) }}, {{ point2[1].toFixed(4) }}]</span>
            </div>
          </div>
        </div>
          ...
          <template v-else-if="cutStep === 2">
            <div class="undo-redo-group">
              <button 
                ...
                @click="undoCut"
                :disabled="!canUndo"
                :title="canUndo ? 'Undo cut' : 'Nothing to undo'"
              >
              ...
              <button 
                ...
                @click="redoCut"
                :disabled="!canRedo"
                :title="canRedo ? 'Redo cut' : 'Nothing to redo'"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
  ...
  methods: {
    ...
    removeHighlight() {
      ...
      if (this.highlightedPolygon && this.layer3) {
        ...
        const originalStyle = this.highlightedPolygon.get('_original_style');
        if (originalStyle) {
      ...
      if (this.selectedPolygon && this.selectedPolygon !== this.highlightedPolygon && this.layer3) {
    ...
    undoCut() {
      ...
      setTimeout(() => {
        ...
        const restoredPolygon = features.find(f => {
          const id = f.get('polygon_id') || f.get('id') || f.get('poly_id');
          return id && id.toString() === this.backupOriginalPolygonId?.toString();
        });
        ...
        if (restoredPolygon) {
          ...
          this.selectedPolygonGeoJSON = this.geoJSONFormat.writeFeatureObject(restoredPolygon, {
            featureProjection: 'EPSG:4326',
            dataProjection: 'EPSG:4326'
          });
        }
    ...
    async saveCut() {
      ...
      try {
        ...
        const originalPolygonIds = this.backupOriginalPolygonId ? [this.backupOriginalPolygonId] : [];
        
        // Find new polygon IDs (from cut features)
        const newPolygonIds = this.cutFeatures.map(f => f.get('polygon_id') || f.get('id'));
        ...
        const response = await fetch(`/api/proposal/${this.proposalId}/save_cut_geometry/`, {
          method: 'POST',
          headers: {
        ...
        if (data.success) {
          ...
          this.errorMessage = data.error || 'Failed to save cut geometry';
