...
class RowMaximizer:
    ...
    def maximize_row(self):
        ...
        for i, (original_fig, original_canvas, original_frame, gdf, title, description) in enumerate(self.row_charts_data):
            # Create frame for this chart with individual popup button
            chart_container = tk.Frame(scrollable_frame, bg='white')
            chart_container.grid(row=1, column=i, padx=10, pady=10, sticky="nsew")
...
def create_scrollable_charts_with_horizontal_scroll(*row_lists, row_descriptions=None, chart_titles=None, chart_descriptions=None):
    """
    ...
    *row_lists: Variable number of lists, each containing GeoDataFrames for a row
    ...
    for row_idx, row_gdfs in enumerate(row_lists):
        ...
        for col_idx, gdf in enumerate(row_gdfs):
            ...
            chart_container = tk.Frame(scrollable_frame)
            chart_container.grid(row=chart_row, column=chart_col, padx=5, pady=5, sticky="nsew")

            # Add individual chart popup button
            chart_title = f"Chart {row_idx + 1}-{col_idx + 1}"
            if chart_titles and row_idx < len(chart_titles) and col_idx < len(chart_titles[row_idx]):
                chart_title = chart_titles[row_idx][col_idx]

            chart_description = ""
            if chart_descriptions and row_idx < len(chart_descriptions) and col_idx < len(chart_descriptions[row_idx]):
                chart_description = chart_descriptions[row_idx][col_idx]
            ...
            if row_idx not in row_data:
                row_data[row_idx] = []
            row_data[row_idx].append((fig, chart_canvas, chart_frame, gdf, chart_title, chart_description))
    ...
    instructions = tk.Label(root,
                          text="Scroll: Mouse Wheel (vertical) | Shift+Mouse Wheel (horizontal) | Arrow Keys | Click 📊 for individual chart popup",
...
def example_usage():
    ...
    try:
        ...
        # Create sample GeoDataFrames with polygons
        ...
        descriptions = [
            "Polygon features with index annotations at representative points"
        ]

        titles = [
            ["Sample Polygons Set 1", "Sample Polygons Set 2"]
        ]

        chart_descs = [
            ["Three annotated polygons showing spatial distribution",
             "Two additional polygons with index labels"]
        ]
        ...
        create_scrollable_charts_with_horizontal_scroll(empty_row1)
...
if __name__ == "__main__":
    ...
    titles = [
        ["Sample Points Set 1", "Sample Points Set 2", "Sample Points Set 1", "Sample Points Set 2"],
        ["Sample Points Set 1", "Sample Points Set 2", "Sample Points Set 1", "Sample Points Set 2"],
        ["Study Area Boundary", "Region of Interest", "Study Area Boundary", "Region of Interest"],
        ["Study Area Boundary", "Region of Interest", "Study Area Boundary", "Region of Interest"]
    ]

    chart_descs = [
        ["Collection of geographic point features showing spatial distribution patterns",
         "Additional point dataset with different spatial characteristics",
         "Additional point dataset with different spatial characteristics",
         "Additional point dataset with different spatial characteristics",
        ],
        ["Primary study area boundary polygon defining the analysis extent",
         "Secondary region of interest for focused analysis",
         "Secondary region of interest for focused analysis",
         "Secondary region of interest for focused analysis",
        ],
        ["Collection of geographic point features showing spatial distribution patterns",
