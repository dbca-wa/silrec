<template>
    <div class="survey-assessment">
        <div v-if="$route.query.debug?.toLowerCase() === 'true'">
            src/components/internal/treatments/survey_assessment.vue
        </div>
        ...
        <div
            class="modal fade"
            :class="{ 'show d-block': showDocumentModal }"
            tabindex="-1"
                                ...
                                <div class="col-md-12">
                                    <div class="form-group mb-1">
                                        ...
                                        <div class="form-text small">
                                            ...
                                            Images, Compressed files. Max size:
                                            50MB
                                        </div>
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                <span
                                                    v-if="
                                                        editingDocument.file_size_display
                                                    "
                                                    class="ms-1"
                                                    >({{
                                                        editingDocument.file_size_display
                                            ...
                                            <div class="form-text small mt-1">
                                                Existing file. Upload a new file
                                                to replace.
                                            </div>
                                        </div>
                                    </div>
    ...
    methods: {
        async loadDocuments() {
            try {
                const response = await fetch(
                    `${api_endpoints.survey_assessment_documents}?treatment_id=${this.treatmentId}`
                );
                if (!response.ok) {
                ...
                await this.showError(
                    'Load Failed',
                    'Failed to load survey/assessment documents'
                );
            }
        },
        ...
        async loadLookups() {
            try {
                const [typesResponse, statusesResponse] = await Promise.all([
                    fetch(
                        `${api_endpoints.survey_assessment_documents}document_types/`
                    ),
                    fetch(
                        `${api_endpoints.survey_assessment_documents}status_choices/`
                    ),
                ]);
        ...
        async deleteDocument(document) {
            ...
            if (document.marked_deleted) {
                try {
                    const response = await fetch(
                        `${api_endpoints.survey_assessment_documents}${document.document_id}/`,
                        {
                            method: 'PATCH',
                            headers: {
          ...
          `,
                ...
                if (result.isConfirmed) {
                    try {
                        ...
                        console.error(
                            'Error marking document as deleted:',
                            error
                        );
                        await this.showError(
                            'Mark Failed',
                            'Failed to mark document as deleted'
                        );
                    }
                }
            }
        ...
        viewDocument(document) {
            if (document.marked_deleted) {
                this.showInfo(
                    'Document Marked for Deletion',
                    'This document is marked for deletion but can still be viewed.'
                );
            }
        ...
        async downloadDocument(document, openInNewTab = false) {
            try {
                ...
                const response = await fetch(
                    `${api_endpoints.survey_assessment_documents}${document.document_id}/download/`,
                    {
                        headers: {
                            'X-CSRFToken': this.getCSRFToken(),
                        },
                ...
                if (
                    openInNewTab &&
                    this.isFileTypeViewable(document.file_type)
                ) {
        ...
        editDocument(document) {
            if (document.marked_deleted) {
                this.showInfo(
                    'Cannot Edit',
                    'This document is marked for deletion and cannot be edited.'
                );
                return;
            }
        ...
        validateForm() {
            ...
            if (
                this.documentSource === 'url' &&
                !this.documentData.file_url.trim()
            ) {
        ...
        getStatusLabel(status) {
            const statusObj = this.statusChoices.find(
                (s) => s.value === status
            );
