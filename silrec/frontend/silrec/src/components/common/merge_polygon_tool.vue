    ...
    <div class="merge-polygon-tool">
        ...
        <div v-if="isMergeModeActive" class="merge-panel">
            ...
            <div class="merge-panel-content">
                ...
                <div v-if="mergeStep < 2" class="two-column-layout">
                    ...
                    <div class="steps-column">
                        <div
                            ...
                            :class="{
                                active: mergeStep === 1,
                                completed: mergeSelectedFeatures.length >= 1,
                            }"
                        >
                            <div class="step-content">
                                <span class="step-number">1</span>
                                <span class="step-text"
                                    >Select first polygon</span
                                >
                            </div>
                            <span
                                v-if="mergeSelectedFeatures.length >= 1"
                                class="step-check"
                                >✓</span
                            >
                        </div>
                        <div
                            ...
                            :class="{
                                active: mergeStep === 1,
                                completed: mergeSelectedFeatures.length >= 2,
                            }"
                        >
                            <div class="step-content">
                                <span class="step-number">2</span>
                                <span class="step-text"
                                    >Select second polygon</span
                                >
                            </div>
                            <span
                                v-if="mergeSelectedFeatures.length >= 2"
                                class="step-check"
                                >✓</span
                            >
                        </div>
                    ...
                    <div class="values-column">
                        <div
                            class="value-row"
                            :class="{ 'has-value': mergeSelectedFeatures[0] }"
                        >
                            <span class="value-label">ID1:</span>
                            <span class="value-content">{{
                                getPolygonId(mergeSelectedFeatures[0]) || '—'
                            }}</span>
                        </div>
                        <div
                            class="value-row"
                            :class="{ 'has-value': mergeSelectedFeatures[1] }"
                        >
                            <span class="value-label">ID2:</span>
                            <span class="value-content">{{
                                getPolygonId(mergeSelectedFeatures[1]) || '—'
                            }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div
                    v-if="mergeStep === 2"
                    class="two-column-layout post-merge"
                >
                    ...
                    <div class="values-column">
                        ...
                        <div class="value-row has-value">
                            ...
                            <span class="value-content">{{
                                mergedArea ? mergedArea.toFixed(2) + ' ha' : '—'
                            }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="merge-panel-actions">
                    ...
                    <template v-if="mergeStep < 2">
                        <button class="cancel-btn" @click="cancelMerge">
                            Cancel
                        </button>
                        <button
                    ...
                    <template v-else-if="mergeStep === 2">
                        <div class="undo-redo-group">
                            <button
                                ...
                                :disabled="!canUndo"
                                :title="
                                    canUndo ? 'Undo merge' : 'Nothing to undo'
                                "
                            >
                                <svg
                                    width="14"
                            ...
                            <button
                                ...
                                :disabled="!canRedo"
                                :title="
                                    canRedo ? 'Redo merge' : 'Nothing to redo'
                                "
                            >
                                <svg
                                    width="14"
                        ...
                        <button class="save-btn" @click="saveMerge">
                            Save
                        </button>
                    </template>
                </div>
...
export default {
    ...
    computed: {
        getButtonTitle() {
            return this.isMergeModeActive
                ? 'Exit Merge Mode'
                : 'Merge Polygons Tool';
        },
        canMerge() {
            return (
                this.mergeSelectedFeatures.length === 2 && !this.errorMessage
            );
        },
        canUndo() {
    ...
    methods: {
        ...
        handleMergeClick(feature) {
            ...
            const index = this.mergeSelectedFeatures.findIndex(
                (f) => f === feature
            );
            ...
            if (index !== -1) {
                ...
                if (this.mergeSelectedFeatures.length < 2) {
                    ...
                    this.errorMessage =
                        'You can only select two polygons. Deselect one first.';
                }
            }
        ...
        removeHighlightFromPolygon(feature) {
            if (feature && this.layer3) {
                ...
                const idx = this.highlightedPolygons.findIndex(
                    (f) => f === feature
                );
                if (idx !== -1) {
        ...
        removeHighlights() {
            console.log(
                'Removing all highlights, count:',
                this.highlightedPolygons.length
            );
        ...
        removeMergedPolygonHighlight() {
            ...
            if (this.layer3 && this.mergedFeatureId) {
                const source = this.layer3.getSource();
                const features = source.getFeatures();
                const mergedPolygon = features.find(
                    (f) => f.get('mergeId') === this.mergedFeatureId
                );
        ...
        async performMerge() {
            ...
            try {
                ...
                if (
                    rewound1.geometry.type !== 'Polygon' ||
                    rewound2.geometry.type !== 'Polygon'
                ) {
                ...
                const simplified = turf.simplify(cleanedUnion, {
                    tolerance: 0.00001,
                    highQuality: false,
                });
                ...
                this.backupFeatures = {
                    ...
                    f2: feature2.clone(),
                    originalIds: [
                        this.getPolygonId(feature1),
                        this.getPolygonId(feature2),
                    ],
                };
                ...
                this.map.getView().fit(mergedExtent, {
                    padding: [50, 50, 50, 50],
                    duration: 500,
                });
        ...
        undoMerge() {
            ...
            setTimeout(() => {
                ...
                if (this.backupFeatures && this.backupFeatures.originalIds) {
                    console.log(
                        'Found backup IDs:',
                        this.backupFeatures.originalIds
                    );
                    ...
                    const originalPolygons = features.filter((f) => {
                        ...
                        return (
                            id && this.backupFeatures.originalIds.includes(id)
                        );
                    });

                    console.log(
                        'Found original polygons to highlight:',
                        originalPolygons.length
                    );
        ...
        saveMerge() {
            ...
            fetch(`/api/proposal/${this.proposalId}/save_merged_geometry/`, {
                ...
                body: JSON.stringify({
                    ...
                    original_polygon_ids: this.backupFeatures
                        ? this.backupFeatures.originalIds
                        : [],
                ...
                .then((data) => {
                    if (data.success) {
                        ...
                        this.errorMessage =
                            data.error || 'Failed to save merged geometry';
