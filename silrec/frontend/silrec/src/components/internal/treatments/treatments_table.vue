...
export default {
    ...
    computed: {
        ...
        isTaskDisabled() {
            return (
                !this.lookups.tasksWithClassification
                    ?.classification_table_exists ||
                (this.filterTaskClassificationId !== 'all' &&
                    this.taskClassifications.length === 0)
            );
        },
            ...
            return {
                ...
                ajax: {
                    ...
                    data: function (d) {
                        ...
                        d.filter_task_classification_id =
                            vm.filterTaskClassificationId;
        ...
        taskPlaceholder() {
            ...
            return this.taskSearch === '' || this.taskInputFocused
                ? 'Type to search tasks...'
                : '';
        },
        statusPlaceholder() {
            return this.statusSearch === '' || this.statusInputFocused
    ...
    methods: {
        async loadLookups() {
            ...
            try {
                ...
                this.lookups.tasksWithClassification =
                    data.tasks_with_classification;
                ...
                this.filteredTasks = this.allTasks.filter(
                    (task) =>
                        ...
                        task.classification.id ===
                            parseInt(this.filterTaskClassificationId)
                );
            }
        },
        ...
        filterStatuses() {
            ...
            this.filteredStatuses = this.lookups.treatment_statuses.filter(
                (status) =>
        ...
        filterMachines() {
            ...
            this.filteredMachines = this.lookups.machines.filter(
                (machine) =>
                    (machine.manufacturer &&
                        machine.manufacturer
                            .toLowerCase()
                    ...
                    (machine.machine_type &&
                        machine.machine_type.toLowerCase().includes(searchTerm))
            );
        },
        ...
        refreshData() {
            if (
                this.$refs.treatments_datatable &&
                this.$refs.treatments_datatable.vmDataTable
            ) {
        ...
        async deleteTreatment(treatmentId) {
            ...
            try {
                const response = await fetch(
                    `${api_endpoints.treatments}${treatmentId}/`,
                    {
                        method: 'DELETE',
                        headers: {
...
.treatments-table-container
    ...
    .filter_warning_icon.filter-active {
    color: #dc3545 !important;
}

.treatments-table-container
    .collapsible-component
    .filter_warning_icon.filter-clear {
    color: #28a745 !important;
}
</style>
