...
class ShapefileSliversMerger():
    ...
    def create_gdf_batch(self, savepoint_callback=None):
        '''
        Single-iteration batch version of create_gdf.
        Overlays ALL shapefile polygons against ALL intersecting historical
        polygons at once, cookie-cuts, merges slivers, then builds cohort
        data in memory and writes everything to DB in one final write sequence.
        '''
        list_state = []
        idx_count = 1

        # 1. Get ALL intersecting historical polygons for the entire shapefile
        gdf_hist = self.get_polygons_gdf(self.gdf_shpfile, 'polygon', self.proposal_id)
        gdf_hist.rename(columns={'geom': 'geometry'}, inplace=True)
        gdf_hist.set_geometry('geometry', inplace=True)
        gdf_hist.set_crs(settings.CRS_GDA94, inplace=True)
        print(f'gdf_hist area.sum() {gdf_hist.area.sum()}')

        polygon_ids_hist = gdf_hist.polygon_id.to_list()
        assignments = AssignChtToPly.objects.filter(
            polygon_id__in=polygon_ids_hist, status_current=True
        ).values('polygon_id', 'cohort_id', 'cht2ply_id')
        hist_cht_map = {a['polygon_id']: {'cohort_id': a['cohort_id'], 'cht2ply_id': a['cht2ply_id']} for a in assignments}

        # 2. initialise store with base data
        list_state.append({
            "GDF_HIST": gdf_hist.copy(),
            "GDF_SHP": self.gdf_shpfile.copy(),
        })

        proposal_obj = Proposal.objects.get(id=self.proposal_id)
        user = User.objects.get(id=self.user_id)
        self.request_metrics = RequestMetrics.objects.create(proposal=proposal_obj, user=user)

        # 3. Single overlay: cookie-cut ALL shapefile polygons against ALL hist polygons
        #    Partition: union of all shp polygons vs all intersecting hist polygons
        shp_union = self.gdf_shpfile.unary_union
        intersects_all = gdf_hist.geometry.intersects(shp_union)
        gdf_hist_subset = gdf_hist[intersects_all].copy()

        gdf_partitioned = gpd.overlay(
            self.gdf_shpfile[['geometry']], gdf_hist_subset,
            how='union', keep_geom_type=True
        )
        gdf_partitioned = gdf_partitioned[gdf_partitioned.area > 1]
        gdf_partitioned = gdf_partitioned.explode(index_parts=False).explode(index_parts=False)
        gdf_partitioned.reset_index(inplace=True, drop=True)

        # 4. Classify each partitioned polygon:
        #    - BASE: representative point falls inside a shapefile polygon
        #    - HIST: representative point falls inside a hist polygon only
        base_polygon_gdf = self.gdf_shpfile.copy()
        centroids = gpd.GeoDataFrame(
            geometry=gdf_partitioned.geometry.representative_point(),
            crs=gdf_partitioned.crs
        )
        centroids['idx'] = range(len(centroids))

        # Join centroids with shapefile polygons to identify BASE polygons
        sjoined_shp = gpd.sjoin(centroids, base_polygon_gdf[['geometry']], how='left', predicate='intersects')
        is_base = ~sjoined_shp['index_right'].isna()
        gdf_partitioned['poly_type'] = 'HIST'
        gdf_partitioned.loc[is_base.values, 'poly_type'] = 'BASE'

        # 5. Identify slivers and merge into BASE polygons
        self.threshold = self.threshold if self.threshold else settings.SLIVER_AREALENGTH_THRESHOLD

        # slivers: polygons entirely outside base polygons with low area/length ratio
        sliver_mask = gdf_partitioned.geometry.area / gdf_partitioned.geometry.length < self.threshold
        gdf_slivers = gdf_partitioned[sliver_mask].copy()
        gdf_slivers['poly_type'] = 'SLVR'

        # Non-sliver hist polygons
        gdf_non_slivers = gdf_partitioned[~sliver_mask].copy()

        # Merge slivers into BASE polygons by dissolving each base polygon
        gdf_base = gdf_non_slivers[gdf_non_slivers.poly_type == 'BASE'].copy()
        gdf_hist_clean = gdf_non_slivers[gdf_non_slivers.poly_type == 'HIST'].copy()

        # For each base polygon, absorb overlapping/intersecting slivers
        if len(gdf_slivers) > 0 and len(gdf_base) > 0:
            gdf_slivers_dissolved = gdf_slivers.dissolve()
            gdf_base_original = gdf_base.copy()
            gdf_base = gpd.overlay(
                gdf_base[['geometry']], gdf_slivers_dissolved[['geometry']],
                how='union', keep_geom_type=True
            )
            gdf_base = gdf_base.explode(index_parts=False).explode(index_parts=False)
            gdf_base.reset_index(inplace=True, drop=True)
            gdf_base['poly_type'] = 'BASE'
            gdf_base['polygon_id'] = 0
            gdf_base['proposal_id'] = self.proposal_id

        gdf_result = gpd.GeoDataFrame(
            pd.concat([gdf_hist_clean, gdf_base], ignore_index=True)
        )
        gdf_result = gdf_result[gdf_result.area > 1]
        gdf_result.reset_index(inplace=True, drop=True)

        # 6. Spatial join: assign polygon_id from hist polygons that each result
        #    polygon's centroid falls into.  BASE polygons get polygon_id=0.
        centroids_res = gpd.GeoDataFrame(
            geometry=gdf_result.geometry.representative_point(),
            crs=gdf_result.crs
        )
        centroids_res['idx'] = range(len(centroids_res))

        gdf_hist_no_geom = gdf_hist_subset.drop(columns=['geometry'], errors='ignore')
        sjoined = gpd.sjoin(centroids_res, gdf_hist_no_geom, how='left', predicate='intersects')

        gdf_result['polygon_id'] = sjoined['polygon_id'].fillna(0).astype(int)
        gdf_result['name'] = sjoined['name'].fillna('')
        gdf_result['compartment'] = sjoined['compartment'].fillna(0).astype(int)
        gdf_result['sp_code'] = sjoined['sp_code'].fillna('')
        gdf_result['area_ha'] = gdf_result.area / 10000
        gdf_result['proposal_id'] = self.proposal_id

        # Drop columns that may cause issues downstream
        drop_cols = ['index', 'is_sliver', 'sliver_ratio', 'area', 'length',
                     'intersect_area', 'overlap_perc', 'index_right']
        for col in drop_cols:
            if col in gdf_result.columns:
                gdf_result.drop(columns=[col], inplace=True, errors='ignore')

        # Merge small polygons with longest neighbour
        gdf_result = find_and_merge(gdf_result, self.threshold)

        # 7. Assign cohort IDs: map hist polygon_id -> current cohort_id
        gdf_result['cht_id_cur'] = gdf_result['polygon_id'].map(
            lambda pid: hist_cht_map.get(pid, {}).get('cohort_id', 1) if pid > 0 else 1
        ).fillna(1).astype(int)

        # 8. Create cohorts from shapefile data and assign cohort_id per BASE polygon
        gdf_result['cohort_id_new'] = 0
        # Build unique (obj_code, target_ba) combos from the shapefile
        shp_cohorts = {}
        for _, row in self.gdf_shpfile.iterrows():
            key = (row.get('obj_code'), float(row.get('target_ba_', 0)))
            if key not in shp_cohorts:
                shp_cohorts[key] = None
        # Create cohort records in-memory first (capture IDs)
        cohort_records = []
        for (obj_code, target_ba) in shp_cohorts:
        ...
        gdf_result['cht_type'] = 'ORIG'
        gdf_result.loc[gdf_result.poly_type == 'BASE', 'cht_type'] = 'NEW'

        # 9. Build gdf_cht_init (existing cohort links cloned from hist)
        cols_init = ['polygon_id', 'poly_id_new', 'area_ha', 'poly_type', 'cht_type',
                     'cht_id_cur', 'iter_seq', 'proposal_id']
        gdf_result_working = gdf_result.copy()
        gdf_result_working['iter_seq'] = idx_count
        gdf_result_working['poly_id_new'] = 0  # placeholder, set by write_polygons_to_db
        gdf_result_working['area_ha_orig'] = gdf_result_working['area_ha']

        gdf_cht_init = gdf_result_working[
            (gdf_result_working.polygon_id > 0) & (gdf_result_working.cht_type == 'ORIG')
        ].copy() if len(gdf_result_working) > 0 else gpd.GeoDataFrame()

        # Query DB for cohort detail data (for gdf_cht_init enrichment)
        # Only query if there are OHER polygon_ids with current assignments
        hist_poly_ids = gdf_hist_subset.polygon_id.unique().tolist()
        hist_poly_ids = [int(x) for x in hist_poly_ids if x > 0]
        cohort_gdf_init = None
        if hist_poly_ids:
            try:
                cht_ids = list(set(
                    a.get('cohort_id', 0) for a in assignments
                ))
                cht_ids = [c for c in cht_ids if c > 0]
                if cht_ids:
                    query = f"""
                        SELECT
                            tp.polygon_id,
                        ...
                        WHERE tactp.cohort_id=ANY(ARRAY[{','.join(map(str, cht_ids))}])
                          AND tactp.status_current=True
                    """
                    with connection.cursor() as cursor:
                ...
                logger.error(f'cohort_gdf_init query error: {e}')

        if cohort_gdf_init is not None and not gdf_cht_init.empty:
            gdf_cht_init['cht_id_cur'] = pd.to_numeric(gdf_cht_init['cht_id_cur'], errors='coerce').fillna(0).astype(int)
            cohort_gdf_init['cohort_id'] = pd.to_numeric(cohort_gdf_init['cohort_id'], errors='coerce').fillna(0).astype(int)
            ...
            for col in ['obj_code', 'polygon_id']:
                if col + '_x' in gdf_cht_init.columns:
                    gdf_cht_init.drop(columns=[col + '_x'], inplace=True)
                    gdf_cht_init.rename(columns={col + '_y': col}, inplace=True)
            cols_reqd = ['cohort_id', 'polygon_id', 'cht2ply_id', 'name', 'area_ha_orig',
                         'obj_code', 'complete_date', 'target_ba_m2ha', 'op_id', 'status_current']
            cols_present = [c for c in cols_reqd if c in gdf_cht_init.columns]
            gdf_cht_init = gdf_cht_init[cols_present]
        ...
        # 10. Build gdf_cht_new (new assignments to create)
        gdf_new_assignments = gdf_result_working[gdf_result_working.cht_type == 'NEW'].copy()
        gdf_new_assignments['status_current'] = True
        gdf_new_assignments['desc'] = 'ASSIGN'

        # For BASE polygons: assign to NEW cohort, set old assignment status_current=False
        gdf_new_base = gdf_new_assignments[gdf_new_assignments.poly_type == 'BASE'].copy()
        gdf_new_base = gdf_new_base.rename(columns={'cht_id_cur': 'cohort_id'})

        gdf_new_base_old = gdf_new_base.copy()
        gdf_new_base_old['status_current'] = False
        gdf_new_base_old['desc'] = 'OLD-BASE_OLD'

        gdf_cht_new = pd.concat([gdf_new_base, gdf_new_base_old], ignore_index=True) if len(gdf_new_base) > 0 else pd.DataFrame()
        gdf_new_cut = gdf_new_assignments[gdf_new_assignments.poly_type == 'BASE'].copy()
        gdf_new_cut = gdf_new_cut.rename(columns={'cht_id_cur': 'cohort_id'})
        if len(gdf_new_cut) > 0:
            if len(gdf_cht_new) > 0:
                gdf_cht_new = pd.concat([gdf_cht_new, gdf_new_cut], ignore_index=True)
            else:
                gdf_cht_new = gdf_new_cut

        # 11. Store intermediate results in list_state for inspection
        list_state.append({
            "GDF_HIST": gdf_hist_subset.copy(),
            "GDF_SINGLE": self.gdf_shpfile.copy(),
            "GDF_RESULT": gdf_result.copy(),
            "GDF_CHT_INIT": gdf_cht_init.copy() if isinstance(gdf_cht_init, pd.DataFrame) else pd.DataFrame(),
            "GDF_CHT_NEW": gdf_cht_new.copy() if isinstance(gdf_cht_new, pd.DataFrame) else pd.DataFrame(),
        })

        # 12. Single write sequence: write polygons -> write cohorts -> write assign_cht_to_ply
        try:
            with transaction.atomic():
                with reversion.create_revision() as revision:
                    # 12a. Write cohort records
                    cohort_id_map = {}  # (obj_code, target_ba) -> cohort_id
                    for rec in cohort_records:
                        cid = write_cohort_to_db(
                            rec['obj_code'], rec['op_id'], rec['year'],
                            rec['target_ba'], rec['regen_method'],
                            self.request_metrics, idx_count, revision
                        )
                        cohort_id_map[(rec['obj_code'], rec['target_ba'])] = cid

                    # 12b. Assign new cohort_id to BASE polygons via spatial join
                    centroids_final = gpd.GeoDataFrame(
                        geometry=gdf_result.geometry.representative_point(),
                        crs=gdf_result.crs
                    )
                    centroids_final['idx'] = range(len(centroids_final))
                    gdf_shp_enriched = self.gdf_shpfile.copy()
                    if 'index_right' in gdf_shp_enriched.columns:
                        gdf_shp_enriched.drop(columns=['index_right'], inplace=True)

                    sjoined_cht = gpd.sjoin(centroids_final, gdf_shp_enriched, how='left', predicate='intersects')
                    gdf_result['cohort_id_new'] = sjoined_cht.apply(
                        lambda r: cohort_id_map.get(
                            (r.get('obj_code'), float(r.get('target_ba_', 0))), 0
                        ) if pd.notna(r.get('index_right')) else 0,
                        axis=1
                    ).fillna(0).astype(int)

                    gdf_result['cht_id_new'] = gdf_result['cht_id_cur']
                    base_mask = gdf_result.poly_type == 'BASE'
                    gdf_result.loc[base_mask, 'cht_id_new'] = gdf_result.loc[base_mask, 'cohort_id_new']
                    cut_new_mask = (gdf_result.poly_type == 'CUT') & (gdf_result.polygon_id != gdf_result['poly_id_new']) if 'poly_id_new' in gdf_result.columns else False
                    if isinstance(cut_new_mask, pd.Series):
                        gdf_result.loc[cut_new_mask, 'cht_id_new'] = gdf_result.loc[cut_new_mask, 'cohort_id_new']

                    # Rebuild gdf_cht_init with poly_id_new from gdf_result
                    gdf_result['poly_id_new'] = gdf_result['polygon_id']
                    gdf_result['area_ha_orig'] = gdf_result['area_ha']

                    gdf_cht_init_final = gdf_result[
                        (gdf_result.polygon_id > 0) & (gdf_result.cht_type == 'ORIG')
                    ][['polygon_id', 'poly_id_new', 'area_ha', 'cht_id_cur', 'poly_type', 'cht_type']].copy() if len(gdf_result) > 0 else pd.DataFrame()
                    if len(gdf_cht_init_final) > 0:
                        gdf_cht_init_final['iter_seq'] = idx_count
                        gdf_cht_init_final['proposal_id'] = self.proposal_id

                    # Rebuild gdf_cht_new with actual IDs
                    gdf_cht_new_list = []

                    # ORIG-CUT: existing poly_id, new poly_id may differ, keep old cohort
                    orig_cut = gdf_result[
                        (gdf_result.cht_type == 'ORIG') & (gdf_result.poly_type == 'CUT')
                    ].copy() if len(gdf_result) > 0 else pd.DataFrame()
                    if len(orig_cut) > 0:
                        orig_cut['status_current'] = True
                        orig_cut['desc'] = 'ORIG-CUT_Y'
                        orig_cut = orig_cut.rename(columns={'cht_id_new': 'cohort_id'})
                        gdf_cht_new_list.append(orig_cut[['poly_id_new', 'cohort_id', 'status_current', 'desc']])

                    # NEW-BASE: assign to NEW cohort
                    new_base = gdf_result[
                        (gdf_result.cht_type == 'NEW') & (gdf_result.poly_type == 'BASE')
                    ].copy() if len(gdf_result) > 0 else pd.DataFrame()
                    if len(new_base) > 0:
                        new_base['status_current'] = True
                        new_base['desc'] = 'NEW-BASE_Y'
                        new_base = new_base.rename(columns={'cht_id_new': 'cohort_id'})
                        gdf_cht_new_list.append(new_base[['poly_id_new', 'cohort_id', 'status_current', 'desc']])

                    # NEW-BASE old assignments: set to False
                    if len(new_base) > 0:
                        new_base_old = new_base.copy()
                        new_base_old['status_current'] = False
                        new_base_old['desc'] = 'NEW-BASE_N'
                        new_base_old = new_base_old.rename(columns={'cht_id_cur': 'cohort_id'})
                        gdf_cht_new_list.append(new_base_old[['poly_id_new', 'cohort_id', 'status_current', 'desc']])

                    # NEW-CUT: new polygons from CUT with NEW cohort
                    new_cut = gdf_result[
                        (gdf_result.cht_type == 'NEW') & (gdf_result.poly_type == 'CUT')
                    ].copy() if len(gdf_result) > 0 else pd.DataFrame()
                    if len(new_cut) > 0:
                        new_cut['status_current'] = True
                        new_cut['desc'] = 'NEW-CUT_Y'
                        new_cut = new_cut.rename(columns={'cht_id_cur': 'cohort_id'})
                        gdf_cht_new_list.append(new_cut[['poly_id_new', 'cohort_id', 'status_current', 'desc']])

                    gdf_cht_new_final = pd.concat(gdf_cht_new_list, ignore_index=True) if gdf_cht_new_list else pd.DataFrame()

                    # 12c. Write polygons to DB
                    ops_summary, gdf_result_with_ids = write_polygons_to_db(
                        gdf_result, self.request_metrics, idx_count, revision
                    )
                    gdf_result['poly_id_new'] = gdf_result_with_ids['poly_id_new']

                    # 12d. Write cohort assignments to DB
                    if not gdf_cht_new_final.empty:
                        # Update poly_id_new from actual DB-write results
                        new_id_map = dict(zip(
                            gdf_result_with_ids['polygon_id'], gdf_result_with_ids['poly_id_new']
                        ))
                        for df_part in gdf_cht_new_list:
                            if len(df_part) > 0:
                                df_part['poly_id_new'] = df_part['poly_id_new'].map(new_id_map).fillna(df_part['poly_id_new']).astype(int)
                        gdf_cht_new_final = pd.concat(gdf_cht_new_list, ignore_index=True)
                        save_cht_new_to_db(gdf_cht_new_final, self.request_metrics, idx_count, revision)

                    # Update list_state with final computed data
                    list_state[-1]['GDF_RESULT'] = gdf_result.copy()
                    if len(gdf_cht_init_final) > 0:
                        list_state[-1]['GDF_CHT_INIT'] = gdf_cht_init_final.copy()
                    if not gdf_cht_new_final.empty:
                        list_state[-1]['GDF_CHT_NEW'] = gdf_cht_new_final.copy()

                    reversion.set_comment(
                        f'Shapefile batch processing for proposal {self.proposal_id} ({len(self.gdf_shpfile)} polygons)'
                    )

        except Exception as e:
            logger.error(f"Error in batch DB write for proposal {self.proposal_id}: {str(e)}")
            raise

        # 13. Final planar enforcement
        is_planar, fixed_gdf = self.check_planar_enforcement(
            gdf_result, 'batch final', fix_overlaps=True
        )
        if fixed_gdf is not None:
            list_state[0]['GDF_RESULT_COMBINED'] = fixed_gdf
        else:
            list_state[0]['GDF_RESULT_COMBINED'] = gdf_result.copy()
